...
export default function Contacto() {
  ...
  const infoItems = [
    ...
    { icon: <Mail size={18} color="#a8d8f0" />, label: 'Email', value: BRAND.email, href: `mailto:${BRAND.email}` },
          ...
          <div style={styles.infoCard}>
            ...
            {infoItems.map((item, i) => (
              <div key={i} style={styles.infoItem}>
                <div style={{ ...styles.infoIcon, backgroundColor: i === 1 ? 'rgba(168,216,240,0.1)' : 'rgba(91,168,212,0.1)' }}>
                  {item.icon}
                </div>
                <div>
            ...
            <p style={styles.responseTime}>
              ⚡ Tiempo de respuesta promedio: <strong style={{ color: '#a8d8f0' }}>menos de 2 horas</strong> en horario comercial.
            </p>
            ...
            <div style={styles.socials}>
              {[
                ...
                <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" style={styles.socialLink}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.color = '#a8d8f0' }}
        ...
        >
          {sent ? (
            <div style={styles.successBox}>
              <CheckCircle size={48} color="#5ba8d4" />
            ...
            <div style={styles.formCard}>
              ...
              <button onClick={handleSubmit} disabled={sending || !form.nombre || !form.email || !form.mensaje}
                ...
                onMouseEnter={(e) => { if (form.nombre && form.email && form.mensaje) (e.currentTarget as HTMLButtonElement).style.backgroundColor = '#94e05a' }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = '#c1ff72' }}
              >
...
const inputBase: React.CSSProperties = { width: '100%', fontFamily: 'Montserrat, sans-serif', fontWeight: 500, fontSize: '0.9rem', padding: '0.75rem 1rem', borderRadius: '8px', border: '1.5px solid #e5e5e5', backgroundColor: '#fafafa', color: '#2e2e2e', outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.2s ease' }

const styles: Record<string, React.CSSProperties> = {
  section: { backgroundColor: '#f8f9fa', padding: '5rem 1.5rem 4rem' },
  ...
  infoCol: {},
  infoCard: { backgroundColor: '#1e1e1e', borderRadius: '16px', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' },
  ...
  infoValue: { fontFamily: 'Montserrat, sans-serif', fontWeight: 500, fontSize: '0.875rem', color: '#cecece', margin: 0, textDecoration: 'none' },
  ...
  fieldLabel: { fontFamily: 'Montserrat, sans-serif', fontWeight: 600, fontSize: '0.78rem', color: '#4b4b4b', textTransform: 'uppercase', letterSpacing: '0.06em' },
  input: inputBase,
  select: { ...inputBase, appearance: 'none' },
  textarea: { ...inputBase, resize: 'vertical', minHeight: '100px' },
  submitBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', backgroundColor: '#c1ff72', color: '#1e1e1e', fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '0.95rem', padding: '0.9rem 1.5rem', borderRadius: '8px', border: 'none', transition: 'background-color 0.2s ease', width: '100%' },
  ...
  successTitle: { fontFamily: 'Montserrat, sans-serif', fontWeight: 800, fontSize: '1.4rem', color: '#2e2e2e', margin: 0 },
  successText: { fontFamily: 'Montserrat, sans-serif', fontWeight: 400, fontSize: '0.95rem', color: '#727376', lineHeight: 1.7, margin: 0 },
  successBtn: { fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '0.875rem', color: '#1e1e1e', backgroundColor: '#c1ff72', border: 'none', borderRadius: '8px', padding: '0.75rem 1.5rem', cursor: 'pointer', marginTop: '0.5rem' },
}
